<h1 align="center">Hi 👋, I'm Mannan Rishi</h1>

<h3 align="center">
AI Engineer • Full-Stack Web Developer • Machine Learning Enthusiast
</h3>

## 🚀 About Me

🎓 Computer Science student passionate about building intelligent software.

🤖 Specialized in Artificial Intelligence, Machine Learning, Computer Vision and Intelligent Automation.

🌐 Experienced in modern Web Development including WordPress, Shopify, Squarespace, FastAPI and scalable backend systems.

💡 I enjoy transforming real-world problems into AI-powered solutions through clean architecture and modern technologies.

🚀 My goal is to become an AI Engineer building impactful products for millions of users.

## 💻 Tech Stack

### Languages

Python

JavaScript

HTML

CSS

SQL

---

### AI & ML

TensorFlow

PyTorch

OpenCV

Scikit-Learn

NumPy

Pandas

---

### Backend

FastAPI

Flask

REST APIs

---

### Web Technologies

WordPress

Shopify

Squarespace

WooCommerce

Elementor

---

### Tools

Git

GitHub

VS Code

Google Colab

## 🚀 Featured Projects

### 🤖 VisionAI

Multi-model AI Image Classification Platform built using FastAPI & PyTorch.

---

### 🧠 AI Jarvis Assistant

Voice-controlled desktop AI assistant with automation and computer vision.

---

### 🛡️ SentinelAI

Smart surveillance suite featuring face verification, weapon detection, and threat monitoring.

---

### 💻 Mini Compiler

Educational compiler visualization platform built using Python & Flask.

---

### 👾 Pac-Man Clone

Classic Pac-Man game developed with Python and Pygame featuring custom ghost AI.

## 🌱 Currently Working On

- AI-powered Applications
- Deep Learning
- FastAPI
- Intelligent Automation
- Research-based AI Projects
- Computer Vision

## 📫 Connect With Me

📧 mannan.rishi.dev@gmail.com

💼 LinkedIn: www.linkedin.com/in/mannan-rishi-69275b333

## Quote

> "Code. Learn. Build. Repeat."
